...
class Booking(models.Model):
    ...
    booked_by = models.ForeignKey(User, related_name="user_bookings", on_delete=models.CASCADE)
    ticket = models.ForeignKey(Ticket, related_name="ticket_bookings", on_delete=models.CASCADE)
...
def send_cancellation_email(sender, instance, *args, **kwargs):
    ...
    if instance.cancelled:
        contact_emails = list(Purchase.objects.filter(ticket__flight=instance).values_list('contact_email', flat=True))
        send_mail("FLIGHT CANCELLED", "SORRY YOUR FLIGHT HAS BEEN CANCELLED", EMAIL_HOST_USER, contact_emails)
