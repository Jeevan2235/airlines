...
from core.infrastructure.check_permissions import (
    TicketFlightPermissions,
    PurchasePermissions,
)
...
class FilteredTicketViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = serializers.FilteredTicketSerializer
    http_method_names = [
        "get",
    ]
    ...
    def get_queryset(self):
        ...
        if len(filters):
            ...
            return models.Ticket.objects.filter(
                flight__in=flight, booked=False, flight__cancelled=False
            )
        return models.Ticket.objects.filter(booked=False, flight__cancelled=False)
