<template>
  <div class="pa-5">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Booked Flight List</v-toolbar-title>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "Email", value: "booking_user_details.email" },
      { text: "Name", value: "booking_user_details.name" },
      { text: "Phone", value: "booking_user_details.phone" },
      {
        text: "Flight Time",
  ...
  methods: {
    ...
    async deleteBooked() {
      const url = "/bookings/cancel-booking/" + this.defaultItem.id + "/";
      await this.$axios
        .post(url)
        .then((response) => {
